...
def merge_data(filename):
    ...
    DEMO_cols = ['SEQN', 'RIDAGEYR', 'RIAGENDR', 'RIDRETH1', 'WTINT2YR', 'DMDEDUC2', 'INDHHINC']
    ...
    df_00_undiag = df_00.merge(DIQ.loc[DIQ.DIQ010 == 2, DIQ_cols] \
                               .merge(LAB10AM.loc[LAB10AM.LBXGLU >= 126, LAB_cols], on='SEQN'),
                           on='SEQN')
    df_02_undiag = df_02.merge(DIQ_B.loc[DIQ_B.DIQ010 == 2, DIQ_cols] \
                               .merge(L10AM_B.loc[L10AM_B.LBXGLU >= 126, LAB_cols], on='SEQN'),
                            on='SEQN')
    df_04_undiag = df_04.merge(DIQ_C.loc[DIQ_C.DIQ010 == 2, DIQ_cols] \
                               .merge(L10AM_C.loc[L10AM_C.LBXGLU >= 126, LAB_cols], on='SEQN'),
                            on='SEQN')
    ...
    df_00_prediab = df_00.merge(LAB10AM.loc[(LAB10AM.LBXGLU >= 100) & \
                                            (LAB10AM.LBXGLU <= 125),
                                            LAB_cols], on='SEQN')
    df_02_prediab = df_02.merge(L10AM_B.loc[(L10AM_B.LBXGLU >= 100) & \
                                            (L10AM_B.LBXGLU <= 125),
                                            LAB_cols], on='SEQN')
    df_04_prediab = df_04.merge(L10AM_C.loc[(L10AM_C.LBXGLU >= 100) & \
                                            (L10AM_C.LBXGLU <= 125),
                                            LAB_cols], on='SEQN')
    ...
    '''
    df_00_nodiab = df_00.merge(LAB10AM.loc[LAB10AM.LBXGLU < 100, LAB_cols], on='SEQN')
    df_02_nodiab = df_02.merge(L10AM_B.loc[L10AM_B.LBXGLU < 100, LAB_cols], on='SEQN')
    df_04_nodiab = df_04.merge(L10AM_C.loc[L10AM_C.LBXGLU < 100, LAB_cols], on='SEQN')
    ...
    df.drop(['LBXGLU','DIQ010'], axis=1).to_csv(filename, index=False)
